...
function drilldownmenu(setting){
	this.sublevelarrow={src:'../WEB-INF/lib/DrillDownMenu/right.gif', width:'8px', top:'3px', left:'6px'} //full URL to image used to indicate there's a sub level
	this.breadcrumbarrow='../WEB-INF/lib/DrillDownMenu/right.gif' //full URL to image separating breadcrumb links (if it's defined)
...
drilldownmenu.prototype.buildmenu=function($){
	...
	this.$uls.each(function(i){ //loop through each UL
		...
		if (i==0){ //if topmost UL
			...
			$('<li class="backcontrol"><img src="'+thisdrill.backarrow+'" style="border-width:0" /> Menu Utama</li>') //add back LI item
